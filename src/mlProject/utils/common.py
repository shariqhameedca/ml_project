...
def create_directories(paths_to_directories: list, verbose=True):
    ...
    Args:
        paths_to_directories (list): A list of paths of directories
    """
...
def save_json(path: Path, data: dict):
    ...
    Args:
        ...
        data (dict): dictionary to be saved as json file 
    """
    with open(path, "w") as f:
        json.dump(data, f, indent=4)

    logger.info(f"Successfully saved json file at {path}")
...
def load_bin(path: Path) -> Any:
    ...
    Args:
        path (Path): path to the bin file to be read
